...
import { AdminFaqService } from '../../../../../src/modules/faq/admin/services/faq.service';
...
describe('AdminFaqService', () => {
  ...
  beforeEach(() => {
    ...
    service = new AdminFaqService(faqRepo as any, redis as any);
  });
  ...
  describe('getOne', () => {
    it('should return item when found', async () => {
      ...
      expect(faqRepo.findById).toHaveBeenCalledWith(1n);
    });

    it('should throw NotFoundException when not found', async () => {
      faqRepo.findById.mockResolvedValue(null);
  ...
  describe('create', () => {
    it('should create with defaults and clear cache', async () => {
      const dto = { question: 'What?', answer: 'This.' };
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when item not found', async () => {
      faqRepo.findById.mockResolvedValue(null);
      await expect(service.update(1n, { question: 'X' } as any)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('delete', () => {
    it('should delete and clear cache', async () => {
      ...
      expect(faqRepo.delete).toHaveBeenCalledWith(1n);
      expect(redis.del).toHaveBeenCalled();
    });
